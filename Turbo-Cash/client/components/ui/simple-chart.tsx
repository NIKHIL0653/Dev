...
export function ComparisonBarChart({
  ...
  const chartHeight = height - 30; // More compact spacing

  return (
    <div className="w-full bg-white rounded-lg p-3">
      <div style={{ height }} className="relative">
        <div className="flex items-end justify-between h-full space-x-4 pb-6">
          {data.map((item, index) => (
            <div
              key={index}
            ...
            >
              ...
              <div className="flex items-end space-x-1 h-full">
                ...
                <div className="flex flex-col items-center">
                  <div
                    className="w-6 bg-violet-500 rounded-t-md transition-all duration-300 hover:opacity-80 relative group"
                    style={{
                      height: `${item.income > 0 ? (item.income / maxValue) * (chartHeight - 30) : 3}px`,
                      minHeight: "3px",
                    }}
                  >
                    {/* Tooltip */}
                    <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-violet-600 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                      {currencySymbol}
                ...
                <div className="flex flex-col items-center">
                  <div
                    className="w-6 bg-purple-500 rounded-t-md transition-all duration-300 hover:opacity-80 relative group"
                    style={{
                      height: `${item.expense > 0 ? (item.expense / maxValue) * (chartHeight - 30) : 3}px`,
                      minHeight: "3px",
                    }}
                  >
                    {/* Tooltip */}
